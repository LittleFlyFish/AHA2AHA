...
import copy
import pyximport;
pyximport.install(setup_args={"script_args":["--compiler=data_utils_cvt"],
                              "include_dirs":np.get_include()},
                  reload_support=True)

####################################################################
# Soe 's change
# add the content in the install to make it work
#################################################################
    ...
    for action_idx in np.arange(len(actions)):
      ...
      for subact in [1, 2]:  # subactions
        ...
        if one_hot:
          ...
          # the odd training data
          the_sequence = np.zeros((len(odd_list), d + nactions), dtype=float)
          the_sequence[:, 0:d] = action_sequence[odd_list, :]
          the_sequence[:, d + action_idx] = 1
          #trainData[(subj, action, subact, 'odd')] = the_sequence
        else:
